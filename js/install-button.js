class e extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (this.renderRoot = this.attachShadow({
                mode: "open"
            }), !e.isSupported || !e.isAllowed) return this.toggleAttribute("install-unsupported", !0), void(this.renderRoot.innerHTML = e.isAllowed ? "<slot name='unsupported'>Ваш браузер не поддерживает установку вещей на устройства ESP. Используйте Google Chrome или Microsoft Edge.</slot>" : "<slot name='not-allowed'>Вы можете устанавливать устройства ESP только на веб-сайты HTTPS или на локальный хост.</slot>");
